pipeline {
    ...
    environment {
        ...
        SSH_PRIVATE_KEY_PATH = '/var/lib/jenkins/.ssh/MY_AWS_KEY.pem'
        ...
        stage('Update Kubernetes manifests and apply') {
            steps {
                script {
                    ...
                    // Copy manifests to K8s master and apply using private key
                    sh """
                        scp -o StrictHostKeyChecking=no -i ${SSH_PRIVATE_KEY_PATH} k8s/deployment.yaml k8s/service.yaml ${K8S_MASTER_USER}@${K8S_MASTER_HOST}:/tmp/
                        ssh -o StrictHostKeyChecking=no -i ${SSH_PRIVATE_KEY_PATH} ${K8S_MASTER_USER}@${K8S_MASTER_HOST} 'kubectl apply -f /tmp/deployment.yaml && kubectl apply -f /tmp/service.yaml'
                    """
                }
            }
        }
